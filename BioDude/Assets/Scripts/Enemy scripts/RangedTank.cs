    ...
    public class RangedTank : Tank
    {
        ...
        private void Update()
        {
            ...
            if (targetInVision) // if enemy can see target
            {
                ...
                Debug.DrawLine(transform.position + head.transform.right * widthOfFirePathChecker,
                    (Vector2) transform.position + (Vector2) head.transform.right * widthOfFirePathChecker +
                    directionToPlayer * distanceToPlayer, Color.blue);
                Debug.DrawLine(transform.position - head.transform.right * widthOfFirePathChecker,
                    (Vector2) transform.position - (Vector2) head.transform.right * widthOfFirePathChecker +
                    directionToPlayer * distanceToPlayer, Color.blue);
                if (!Physics2D.Raycast(transform.position + head.transform.right * widthOfFirePathChecker,
                        directionToPlayer, distanceToPlayer, obstacleMask) &&
                    !Physics2D.Raycast(transform.position - head.transform.right * widthOfFirePathChecker,
                        directionToPlayer, distanceToPlayer, obstacleMask))
                {
                    ai.canMove = false;
                    firearm.Shoot();

...
namespace GUI_scripts
{
    public class PauseMenu : MonoBehaviour
    {
        ...
        private static bool blockPause;
        ...
        private void Start()
        {
            ...
            PauseMenuUI.SetActive(
                false); //disabling pausemenu canvas because it should only be active when pausemenu is summoned
        ...
        private void Update()
        {
            ...
            else if (Input.GetKeyDown(KeyCode.Escape))
            {
                if (PausemenuOpen)
                    Resume();
                else if (!DialogueManager.IsDialogueOpen() && !blockPause)
                    Pause();
            }
        }
        ...
        {
            var lvlManager1 = GameObject.Find("LevelManager").GetComponent<LevelManager>();
            Time.timeScale = 1f;
            lvlManager1.LoadNextLevel();
        }
        ...
        public void ShowNextLevelScreen()
        {
            ...
            if (!lvlManager.LastLevel) return;
